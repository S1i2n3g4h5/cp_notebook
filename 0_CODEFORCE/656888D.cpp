...
(n>2) let's write down all the strings of length n which contain n−2
...
You are given n
(n>2) and k (1≤k≤n⋅(n−1)2). Print the k
...
(1≤t≤104) — the number of test cases in the test. Then t

test cases follow.

Each test case is written on the the separate line containing two integers n
and k (3≤n≤105,1≤k≤min(2⋅109,n⋅(n−1)2)
...
Example*/
    ...
    ll first_b_pos = n - 2 - (sqrt(8*(n*(n-1)/2 - k) + 1) - 1)/2 + 1;
    ll total_before_first_b = (n - first_b_pos)*(n - first_b_pos - 1)/2;
    ll second_b_pos = n - (k - total_before_first_b) + 1;

    f(i,1,n+1){
        if(i == first_b_pos or i == second_b_pos){
            cout << 'b';
        }else{
            cout << 'a';
        }
